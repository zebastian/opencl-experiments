 ...
 * Dies ist ein Testkernel zur Veranschaulichung der Funktionsweise von
 * OpenCL. Zu Beginn werden zwei Arrays A und B definiert. Dann wird OpenCL
 * initialisiert (1. Platform, 1. Gerät) und auf diesem mittels OpenCL
 * die Vektoraddition der beiden Arrays ausgeführt
 * und in ein drittes Array zurückkopiert.
 */
...
int main()
{
	...
	// Dies ist der OpenCL Quellcode der schlussendlich auf dem Gerät ausgeführt
	// wird. Dieser wird als String vorbereitet und enthält den Kernel
	// vecAdd(...) als Beispiel für einen simplen Kernel.
	// get_global_id(0) gibt die aktuelle Position der Range zurück.
	// im Hostcode wäre dies beispielsweise eine Laufvariable i.
	std::string kernelCode =
		"void kernel vecAdd(                                                "
		"  global const int* A, global const int* B, global int* C){        "
		"  C[get_global_id(0)] = A[get_global_id(0)] + B[get_global_id(0)]; "
		"}                                                                  ";

	// Erzeugung eines source Elements zum Laden und Kompilieren des OpenCL
	// Quellcodes
	cl::Program::Sources sources;
	sources.push_back({kernelCode.c_str(), kernelCode.length()});

	// Erzeugung / Kompilierung des Programms aus dem Quelltext im optimiert für
	// den Worker
	cl::Program program(context, sources);
	if (program.build({device}) != CL_SUCCESS)
		errorExit("Kann nicht kompilieren: "
	...
	// Eingabe Arrays A und B in die Queue zur Weitergabe an den Worker
	// schreiben
	...
	// Über den Bereich von "SIZE" den Worker anweisen auf seinen übergebenen
	// Buffern den Kernel anzuwenden
	// Mit dem enqueue und dem anschließenden finish wird das Programm auf der
	// GPU ausgeführt und synchron auf das Ergebnis gewartet.
	queue.enqueueNDRangeKernel(
		kernelVecAdd, cl::NullRange, cl::NDRange(SIZE), cl::NullRange);
	queue.finish();
